...
func (v *Validators) refresh(ctx context.Context) {
	...
	delete(validatorSet, ids.EmptyNodeID) // Ignore inactive validators.
	...
	sortValidators(v.validatorList)

	v.lastUpdated = time.Now()
}

// sortValidators sorts validators by weight (descending) then by nodeID
func sortValidators(validators []validator) {
	for i := 1; i < len(validators); i++ {
		for j := i; j > 0 && validators[j].Compare(validators[j-1]) < 0; j-- {
...
func (v *Validators) Sample(ctx context.Context, limit int) []ids.NodeID {
	...
	for i := 0; i < len(v.validatorList) && len(sampled) < limit; i++ {
